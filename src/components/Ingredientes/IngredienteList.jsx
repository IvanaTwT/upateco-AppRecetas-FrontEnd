    ...
    async function fetchIngredients() {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/reciperover/recipes/${id}/ingredients/`
        );
        ...
        const ingredientPromises = data.map(async (ingR) => {
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/reciperover/ingredients/${ingR.ingredient}/`
          );
  ...
  return (
    <div className="table-container mb-5">
      ...
      {listIng.map((ing, index) => (
              <li key={index}>{ing.name} {ing.quantity}{ing.measure}</li>
          ))}
      </ul>
    </div>
  );
